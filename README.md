...
Cross-platform audio streaming over UDP with Opus encoding. Stream system audio from any client to a server receiver.

> **Note:** Windows support is currently in progress. v1 supports Linux only.
...
If no loopback device is found, set the monitor source manually then run with the `pulse` device:
```bash
